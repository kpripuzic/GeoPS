...
import hr.fer.retrofit.geops.data.indexing.PartitionedSpatialIndexFactory;
...
public class SpisProcessor extends AbstractProcessor {

    public SpisProcessor(String[] args) {
        super(args);
    }

    public static void main(String[] args) throws Exception {
        final AbstractProcessor processor = new SpisProcessor(args);
        ...
        List<Tuple2<Integer, SpatialIndex>> partitionedIndex = PartitionedSpatialIndexFactory.createPairs(IndexType.valueOf(indexType), subscriptions, partitioner);
            ...
            stream.
                    ...
                    transformToPair(
                            rdd -> rdd.
                                    ...
                                    join(indexRDD).
                                    values().
                                    mapToPair(pair -> new Tuple2<>(pair._1, pair._2.query(pair._1.getGeometry().getEnvelopeInternal())))
                    ).
                    //lower pipeline
                    mapToPair(pair -> {
                        Set<Integer> satisfiedSubscriptions = new HashSet<>();

                        for (Subscription candidateSubscription : (List<Subscription>) pair._2) {
